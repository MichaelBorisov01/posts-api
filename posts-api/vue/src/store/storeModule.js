...
export const storeModule = {
  ...
  getters: {
    sortedPosts(state) {
      if (state.selectedSort === 'id') {
        return [...state.posts].sort((post1, post2) =>
          post1[state.selectedSortId]?.localeCompare(
            post2[state.selectedSortId]
          )
        )
      } else {
        return [...state.posts].sort((post1, post2) =>
    ...
    sortedAndSearchedPostsAndUsers(state, getters) {
      ...
      let posts = getters.sortedPosts.filter((post) =>
        ...
        client
          ...
          .then((result) => {
            ...
            commit(
              'setTotalPages',
              Math.ceil(
                Array.from(result.data.posts.data).length / state.limitPost
              )
            )
          })
      } catch (e) {
